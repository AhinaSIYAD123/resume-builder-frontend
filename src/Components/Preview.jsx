...
import { CiSaveDown2, CiClock2, CiPhone, CiMail, CiLocationOn } from "react-icons/ci";
    ...
    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mt: 2 }}>
      <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
        ...
        <Tooltip title="History">
          <Button variant="text" component={RouterLink} to="/history">
            <CiClock2 size={22} />
          </Button>
        </Tooltip>
      </Stack>
      ...
      <Box
        sx={{
          ...
          "& > :not(style)": { m: 2, width: 350, height: "auto" },
        }}
      >
        <Paper id="result" elevation={3} sx={{ p: 3 }}>
          ...
          >
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <CiPhone /> {contactDetails.phoneNumber || "-"}
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <CiMail /> {contactDetails.email || "-"}
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <CiLocationOn /> {personalDetails.location || "-"}
            </span>
          </Typography>
          ...
          >
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <FaGithub /> {contactDetails.github || "-"}
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <FaLinkedin /> {contactDetails.linkedIn || "-"}
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <FaGlobe /> {contactDetails.portfolio || "-"}
            </span>
          </Typography>
          ...
          <Typography variant="body2" align="center" sx={{ mb: 2 }}>
            {educationDetails.course || "Course"}
            <br />
            {educationDetails.college || "College"} | {educationDetails.university || "University"} |{" "}
          ...
          <Typography variant="body2" align="center" sx={{ mb: 2 }}>
            ...
            <br />
            {workExperience.company || "Company"} | {workExperience.location || "Location"} |{" "}
          ...
          <Stack direction="row" spacing={1} justifyContent="center" sx={{ flexWrap: "wrap", gap: 1 }}>
            {skills.length > 0 ? (
              skills.map((skill, i) => (
                <Button key={i} variant="contained" color="primary" size="small">
                  {skill}
                </Button>
              ))
            ) : (
